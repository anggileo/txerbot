...
async def ytdl(_, message):
    ...
    try:
        if userLastDownloadTime > datetime.now():
            ...
            await message.reply_text(f"`Tunggu {wait_time} sebelum lanjut`")
            return
    except:
        pass

    url = message.text.strip()
    await message.reply_chat_action("ngetik")
    try:
        ...
        await message.reply_text("`gagal ambil data 😔 \nmungkin ip diblok yt \n#error`")
        return
    buttons = InlineKeyboardMarkup(list(create_buttons(formats)))
    sentm = await message.reply_text("memproses url")
    try:
